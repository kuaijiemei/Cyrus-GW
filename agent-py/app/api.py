"""对外 HTTP 路由（/agent/chat）：Gateway 之后的业务入口。
...
async def agent_chat(req: AgentChatRequest):
    ...
    if req.stream:
        async def sse_iter() -> AsyncIterator[str]:
            ...
            stream_failed = False
            try:
                async for delta in _core.handle_chat_stream(req):
                    first_delta_sent = True
                    yield _sse_event("delta", {"request_id": request_id, "delta": delta})
                yield _sse_event("done", {"request_id": request_id})
                latency_ms = int((time.perf_counter() - started) * 1000)
                _log_access(request_id, latency_ms, 200, "", "/agent/chat", True, 0, "")
            except LLMConfigError as exc:
                stream_failed = True
                latency_ms = int((time.perf_counter() - started) * 1000)
                _log_access(request_id, latency_ms, 503, "", "/agent/chat", True, 0, "llm")
                yield _sse_event(
                    "error",
                    {
                        "request_id": request_id,
                ...
                stream_failed = True
                latency_ms = int((time.perf_counter() - started) * 1000)
                _log_access(request_id, latency_ms, 504, "", "/agent/chat", True, getattr(exc, "retry_count", 0), "llm")
                yield _sse_event(
                    "error",
                    {
                        "request_id": request_id,
                ...
                stream_failed = True
                latency_ms = int((time.perf_counter() - started) * 1000)
                _log_access(request_id, latency_ms, 502, "", "/agent/chat", True, 0, "llm")
                yield _sse_event(
                    "error",
                    {
                        "request_id": request_id,
    ...
    try:
        ...
        _log_access(resp.request_id, latency_ms, 200, resp.tool_used, "/agent/chat", req.stream, getattr(resp, "retry_count", 0), "")
        return resp
    ...
    # 504：LLM 侧等待超限，与网关等 Agent 超时可同属「超时」族，客户端可重试策略一致。
        ...
        status = 504
        request_id = request_id or "req_unknown"
        _log_access(request_id, latency_ms, status, "", "/agent/chat", req.stream, getattr(exc, "retry_count", 0), "llm")
        return JSONResponse(
            status_code=status,
            content={
                "request_id": request_id,
